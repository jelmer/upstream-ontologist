...
def guess_from_homepage(url: str):
    ...
    try:
        ...
        logger.warning("unable to access homepage %r: %s", url, e)
        return
    except urllib.error.URLError as e:
        logger.warning("unable to access homepage %r: %s", url, e)
        return
    except ConnectionResetError as e:
        logging.warning("unable to access homepage %r: %s", url, e)
        return
    ...
    for a in soup.findAll("a"):
        ...
        for label in filter(None, labels):
            if label.lower() in ("github", "git", "repository", "github repository"):
                yield UpstreamDatum(
                    "Repository", urljoin(basehref, href), certainty="possible"
                )
            if label.lower() in ("github bug tracking", "bug tracker"):
                yield UpstreamDatum(
                    "Bug-Database", urljoin(basehref, href), certainty="possible"
                )
